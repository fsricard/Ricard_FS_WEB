...
if (isset($_GET['accion'], $_GET['id'])) {
    ...
    if ($_GET['accion'] === 'activar') {
        $stmt = $pdo->prepare("UPDATE sarcasmo_frases SET activo = 1 WHERE id = :id");
        $stmt->execute([':id' => $id]);
        $mensaje = mostrarAlerta('Frase activada correctamente.', 'success');
    }

    if ($_GET['accion'] === 'ocultar') {
        $stmt = $pdo->prepare("UPDATE sarcasmo_frases SET activo = 0 WHERE id = :id");
        $stmt->execute([':id' => $id]);
        $mensaje = mostrarAlerta('Frase ocultada.', 'warning');
    }

    if ($_GET['accion'] === 'eliminar') {
        $stmt = $pdo->prepare("DELETE FROM sarcasmo_frases WHERE id = :id");
        $stmt->execute([':id' => $id]);
...
// Total de registros
$stmt = $pdo->prepare("SELECT COUNT(*) FROM sarcasmo_frases $where_sql");
$stmt->execute($params);
...
// Obtener frases
$sql = "SELECT * FROM sarcasmo_frases $where_sql ORDER BY creado_en DESC LIMIT :offset, :limit";
...
<main>
    <section>
        <div class="container">
            <h2>Gestión de las frases sarcásticas del Diablillo</h2>
            ...
            <form method="get" class="filtros">
                <div class="fila">
                    ...
                    <div>
                        ...
                        <button type="button" onclick="window.location='frases_listado.php'">
